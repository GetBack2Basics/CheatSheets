...
Set up Hermes Agent on Linux and enforce per-folder context isolation using direnv and named Hermes profiles.

## Discussion

The Caveman workflow emphasizes strict context boundaries. Each project folder gets an explicit profile so prompts stay short, memory scope remains local, and cross-project contamination risk is reduced.
...
### Part 2: Automated profile creation per folder
...
In Hermes paste the following
    ...
    5. REPORT ONLY ON COMPLETION — Report back ONLY when pipeline finishes or hits fatal runtime error. No interim status messages unless actions are taking longer than 3 mins. In which case issue a quick update of less than 200 characters.
